...
class FinancialAgent:
    def __init__(self, llm: BaseChatModel, tool_descriptions: List[Dict], tool_executor: Callable):
        self.llm = llm
        self.tool_descriptions = [tool_desc for tool_desc in tool_descriptions if tool_desc["name"] in ["market", "policy", "insurance", "agri_fact_search", "weather", "code_interpreter"]]
    ...
    def _build_system_prompt(self) -> str:
        ...
        return f"""You are a specialized financial assistant for agricultural needs. Your role is to provide accurate financial guidance, calculations, and information to farmers.
...
{{\"type\": \"final_response\", \"response\": \"Your detailed financial answer with calculations and recommendations\"}}

WORKFLOW:
1. Analyze the user's financial query
2. If you need external data (market prices, policies, etc.) → make tool calls
3. If you can calculate or answer directly → provide final response
4. After receiving tool results → ALWAYS provide a comprehensive final response

FINANCIAL EXPERTISE:
- Loan calculations (EMI, interest rates, principal)
- Agricultural insurance guidance  
...
Market Query:
{{\"type\": \"tool_calls\", \"tool_calls\": [{{\"name\": \"market\", \"arguments\": {{\"query\": \"wheat prices Uttar Pradesh\"}}}}]}}

Policy Query:
{{\"type\": \"tool_calls\", \"tool_calls\": [{{\"name\": \"policy\", \"arguments\": {{\"query\": \"government schemes small farmers Karnataka paddy\"}}}}]}}

Calculation:
{{\"type\": \"tool_calls\", \"tool_calls\": [{{\"name\": \"code_interpreter\", \"arguments\": {{\"code\": \"# EMI calculation code here\"}}}}]}}

Direct Response:
{{\"type\": \"final_response\", \"response\": \"Hello! I can help with financial calculations, market prices, and government schemes for farmers.\"}}
...
Remember: Return ONLY valid JSON, no additional text."""
    ...
    def _call_agent(self, state: AgentState) -> Dict[str, Any]:
        ...
        if not response.content or response.content.strip() == "":
            ...
            try:
                ...
                # Try to extract tool call info manually if it's a tool call
                if '"type": "tool_calls"' in clean_content and '"name": "weather"' in clean_content:
                    print("DEBUG: Detected incomplete tool call, reconstructing")
                    parsed_response = {
                        "type": "tool_calls",
                        "tool_calls": [{"name": "weather", "arguments": {"lat": 35.6895, "lon": 139.6917}}]
                    }
                else:
    ...
    def _execute_tools(self, state: AgentState) -> Dict[str, Any]:
        ...
        user_prompt = "Please analyze the tool results and provide a comprehensive response."
...
if __name__ == "__main__":
    ...
    agent = FinancialAgent(llm, tool_descriptions, tool_executor_sync)
    print(agent.run("What will my loan EMI be if I take a loan of 500000 for 10 years at 10% interest rate?"))